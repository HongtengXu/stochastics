...
\begin{definition}
  ...
  \begin{equation}
    \Zeta (y_n) = 0 \tmop{and} \zeta \left( \frac{1}{2} + i y_n \right) = 0
...
\subsection{The Argument S(t)}

\begin{figure}[h]
  \resizebox{6in}{3in}{\includegraphics{liftedTwisted.eps}}
  \caption{}
\end{figure}

\begin{figure}[h]
  \resizebox{6in}{3in}{\includegraphics{liftedTwistedAroundFirstBadGramPoints.eps}}
  \caption{}
\end{figure}

\subsubsection{The Gram Points}

Let
\begin{equation}
  g (n) = 2 \pi e^{1 + W \left( \frac{8 n + 1}{8 e} \right)}
\end{equation}
be the approximate value of the $n$-th Gram point. Now define
\begin{equation}
...
1} Z (g (n)) > 0$ and the value $0$ when it is a ``good'' Gram point $(- 1)^{n
+ 1} Z (g (n)) < 0$.
...
\section{A Point Process Model For the Riemann Zeros}

The Riemann zeros have a Hurst exponent of $\sim 0.1$ according to
{\cite{lowRZHurstExponent}}.

\begin{figure}[h]
  \resizebox{850px}{500px}{\includegraphics{zetazeroautocorr.png}}
  \caption{Autocorrelation spectrum of $y_n - y_{n - 1}$ where $y_n$ is the
  $n$-th zero $\zeta \left( \frac{1}{2} + i y_n \right) = 0$}
\end{figure}

If the integral of the autocovariance diverges then that means the
self-exciting point process model it follows is necessarily critical with a
branching rate precisely equal to 1. The branching rate is the integral of the
kernel function over its domain of definition. {\cite{chp}} Since the 1st
several autocorrelations are negative and the remaining ones are positive that
implies that the kernel function which controls their rate of appearance is a
mixture of a repelling component and an attracting component, or in other
words, an inhibiting and an exciting component.
...
\begin{thebibliography}{1}
  \bibitem[1]{chp}Pierre Br{\'e}maud  and  Laurent Massouli{\'e}.
  {\newblock}Hawkes branching point processes without ancestors.
  {\newblock}\tmtextit{J. Appl. Probab.}, 38(1):122--135, 03 2001.{\newblock}
  
  \bibitem[2]{z0t}Guilherme Franca  and  Andre LeClair.
  ...
  \bibitem[8]{ramachandra1995lectures}Kanakanahalli Ramachandra.
  {\newblock}\tmtextit{Lectures on the mean-value and omega-theorems for the
  Riemann zeta-function},  volume~85. {\newblock}Springer, 1995.{\newblock}
  
  \bibitem[9]{lowRZHurstExponent}O Shanker. {\newblock}Explanation of low
  hurst exponent for riemann zeta zeros. {\newblock}2, 06 2011.{\newblock}
\end{thebibliography}

...
public class IndependentMututallyExcitingProcessSimulator {
	...
	static {
		System.setProperty("java.util.concurrent.ForkJoinPool.common.threadFactory",
				TerseThreadFactory.class.getName());
	}

	private AbstractMutuallyExcitingProcess process;

	public IndependentMututallyExcitingProcessSimulator(AbstractMutuallyExcitingProcess process) {
	...
	public static void main(String[] args) throws IOException, CloneNotSupportedException, InterruptedException {
		...
		rangeClosed(0, threadCount - 1).parallel().forEach(thread -> {
			FullyCovariantExtendedMutuallyExcitingExponentialPowerlawApproximationProcess anotherProcess = FullyCovariantExtendedMututallyExcitingExponentialPowerlawApproximationProcessTest
					.constructLongerProcess();
			DiagonalExtendedApproximatePowerlawMututallyExcitingProcess process = new DiagonalExtendedApproximatePowerlawMututallyExcitingProcess(
					2);
			process.ε.set(anotherProcess.ε.diag().get(0), anotherProcess.ε.diag().get(0));
			process.τ.set(anotherProcess.τ.diag().get(0), anotherProcess.ε.diag().get(0));
			process.η.set(anotherProcess.η.diag().get(0), anotherProcess.ε.diag().get(0));
			process.b.set(anotherProcess.b.diag().get(0), anotherProcess.ε.diag().get(0));
	...
	public static Vector simulateProcess(DiagonalExtendedApproximatePowerlawMututallyExcitingProcess process,
			int seed) {
		...
		out.println("simulating " + ansi().fgBrightYellow() + process + ansi().fgDefault() + " from " + process.T.size()
				+ " points with seed=" + seed + " meanRecurrenceTimes=" + process.meanRecurrenceTimeVector());
		double nextTime = 0;
		...
		for (int m = 0; m < process.dim(); m++) {
			for (int i = 0; i < sampleCount; i++) {
				...
				if (dt > 10000 || dt < 0.001) {
					...
					rejects++;
					out.println("seed " + seed + ":" + ansi().fgBrightRed() + "rejecting dt=" + dt + " for y=" + y + "#"
							+ rejects + " points since last reject=" + pointsSinceLastRejection + ansi().fgDefault());
					continue;
				} else {
				...
				if (i % 1000 == 1) {
					String msg = "seed=" + seed + " m=" + m + " i=" + i + " y=" + y + " = q = " + q + " dt=" + dt
							+ " Λmean=" + process.Λ(m).mean() + " Λvar=" + process.Λ(m).variance() + " nextTime="
							+ nextTime + " Edt=" + Edt;
					out.println(msg);
				...
				if (abs(y - q) > 1E-8) {
					out.println(seed + ":" + ansi().fgBrightRed() + " rejecting dt=" + dt + " for y=" + y + " q=" + q
							+ "# " + rejects + ansi().fgDefault());
					continue;
				}
			}
			try {
	...
	public static void
			...
			storeParameterEstimationResults(File testFile, Vector data, AbstractMutuallyExcitingProcess process) {
